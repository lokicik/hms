...
export default function ReportsPage() {
  ...
  const generateReport = async () => {
    try {
      ...
      const emptyRooms = roomsData.filter(room => room.status === 'empty').length;
      const outOfServiceRooms = roomsData.filter(room => room.status === 'out-of-service').length;
      const occupancyRate = totalRooms > 0 ? (occupiedRooms / totalRooms) * 100 : 0;
      
      const calculateRevenue = (occupancyPercent) => {
        const avgPrice = roomsData.reduce((sum, room) => sum + parseFloat(room.basePrice), 0) / totalRooms;
      ...
      if (reportType === 'weekly') {
        for (let i = 0; i < 7; i++) {
          ...
          const dailyOccupancyPercent = totalRooms > 0 ? (activeCounts / totalRooms) * 100 : 0;
  ...
  const handleDownloadCSV = () => {
    ...
    alert('CSV download would start in a real application. This would contain the full report data.');
  };

  const handleDownloadPDF = () => {
    if (!reportData) return;
    
    alert('PDF download would start in a real application. This would be a formatted report.');
  };
    ...
    <AppLayout>
      ...
      {loading ? (
        ...
        <div>
          ...
          <Card 
            title={`${reportType.charAt(0).toUpperCase() + reportType.slice(1)} Occupancy Data`} 
          ...
          >
            <Table 
              ...
              dataSource={reportData.dailyOccupancy.map((item, index) => ({ ...item, key: index }))} 
              pagination={false}
            />
          </Card>
        </div>
