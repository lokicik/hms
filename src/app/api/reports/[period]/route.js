...
export async function GET(request, { params }) {
  try {
    ...
    if (period === 'daily') {
      ...
      startDate.setDate(startDate.getDate() - 3); // Start 3 days before the selected date
...
// Helper function to calculate occupancy rate for a specific date
...
function calculateRevenue(bookings, startDate, endDate) {
  ...
  return relevantBookings.reduce((total, booking) => {
    ...
    // Calculate how many days of this booking fall within our range
    const overlapStart = new Date(Math.max(new Date(booking.checkIn), new Date(startDate)));
    const overlapEnd = new Date(Math.min(new Date(booking.checkOut), new Date(endDate)));
    const overlapDays = Math.ceil((overlapEnd - overlapStart) / (1000 * 60 * 60 * 24));
    
    // Add the revenue for the overlapping days
